...
// Mock fetch to return plox server JSON response
(globalThis as any).fetch = async (url: string) => {
  console.log(`[Test Mock] Intercepted fetch for: ${url}`);
  if (url.includes("realKalos")) {
    return {
      ok: true,
      status: 200,
      json: async () => ({
...
async function runTest() {
  ...
  // Prepare Main Page (Simulation of browser tab)
  ...
  // Trigger Investigation using actual background.ts function
  console.log(
    `[${TEST_DATE}] Triggering performInvestigation for ${TARGET_HANDLE}`,
  );
  await extensionWorker.performInvestigation(
    TARGET_HANDLE,
    1,
    "test-element-id",
  );

  // Verify result sent back to tab
  ...
  console.log(
    `[${TEST_DATE}] Result received from worker: Flag=${msg.flag}, Loc=${msg.location}`,
  );

  if (msg.flag === "🇪🇺" && msg.location === "Eastern Europe (Non-EU)") {
    console.log(
      "🎉 TEST PASSED: Extension logic correctly identified the person's location from server response.",
    );
  } else {
