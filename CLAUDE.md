...
| Action | Command |
|--------|---------|
| Install deps | `cd extension && npm install` |
| Build extension | `cd extension && npm run build` (or `make build`) |
| Run all tests | `cd extension && npm test` (or `make test`) |
| Run a single test | `cd extension && npx playwright test tests/<file>.spec.ts` |
| Format code | `cd extension && ./scripts/format.sh` (or `make format`) |
| Type-check | `cd extension && npx tsc --noEmit` |
| Bootstrap environment | `make setup` |
| Package for distribution | `make dist` |

Build uses esbuild (`extension/build.js`) to bundle `extension/src/background.ts` and `extension/src/content.ts` into `extension/dist/` as IIFE targeting Chrome 121+. The server URL is injected at build time via `PLOX_SERVER_URL` env var (default `https://plox.krepost.xy`).
...
Chrome loads `extension/dist/` as the unpacked extension directory.
...
- Prettier for TypeScript/JS formatting (run via `extension/scripts/format.sh`).
- Extension output in `extension/dist/` — only `manifest.json` and `styles.css` are hand-written; `.js` files are generated by the build.
- Tests mock the plox server responses (JSON) in Playwright route handlers. MHTML fixtures still used for realistic DOM content in `extension/tests/plox.spec.ts`.
