...
test("realistic extension simulation on realKalos account", async ({ page }) => {
    const TEST_DATE = "2025-12-27";
    const mainMhtml = fs.readFileSync(path.join(__dirname, `${TEST_DATE}.mhtml`), "utf8");
    const aboutMhtml = fs.readFileSync(path.join(__dirname, `${TEST_DATE}-about.mhtml`), "utf8");

    const mainHtml = extractHtmlFromMhtml(mainMhtml);
    const aboutHtml = extractHtmlFromMhtml(aboutMhtml);

    if (!mainHtml || !aboutHtml) throw new Error("Could not extract HTML from MHTML");
    ...
    // 3. Inject CSS
    const css = fs.readFileSync(path.join(__dirname, "../extension/styles.css"), "utf8");
    await page.addStyleTag({ content: css });

    // 4. Mock chrome.runtime APIs and inject background logic directly into the page context
    await page.evaluate(({ location, flag }: { location: string, flag: string }) => {
